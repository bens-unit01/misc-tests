...
/** @example ADC example with S110 softdevice
 *
 * @brief This example shows how the internal nRF51 ADC can be operated with the S110 softdevice. 
 * It is implemented as an add on to the Board/nrf6310/ble/ble_app_hrs example in the nRF51 SDK.
 * ADC samples are taken and output on LEDs on the nRFgo motherboard. The example advertises and
 * connects over BLE link as the original ble_app_hrs example in the nRF51 SDK.
 *
 * An additional application timer is
 * set up to trigger the ADC sampling which is performed 10 times per second. ADC is sampled
 ...
 * 11 (led 3) is toggled and the samping result is output on port 2 (pins 16-23).
 *
 * In order to realize changes to the ble_app_hrs to enable the ADC operation, search for "ADC" in this file
 */
...
static void leds_init(void)
{
    ...
    nrf_gpio_cfg_output(ASSERT_LED_PIN_NO);
		...
		nrf_gpio_port_dir_set(NRF_GPIO_PORT_SELECT_PORT2, NRF_GPIO_PORT_DIR_OUTPUT);  //Added output to show ADC result
}
...
{	
	...
	NRF_ADC->CONFIG	= 	 (ADC_CONFIG_PSEL_AnalogInput2 << ADC_CONFIG_PSEL_Pos)					/*!< Use analog input 2 as analog input. */
