...
#define CONNECTED_LED_PIN_NO               MCLR                                           /**< Is on when device has connected. */
//#define CAPS_ON_LED_PIN_NO               LED_2                                          /**< Pin for indicating that CAPS LOCK is on. */
//#define ADV_DIRECTED_LED_PIN_NO          LED_3                                          /**< Is on when we are doing directed advertisement. */
...
#define DEVICE_NAME                      "ww_beacon_01"                              /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                "WowWee"                          /**< Manufacturer. Will be passed to Device Information Service. */
...
static void log_string(uint8_t * str)
{
 SEGGER_RTT_printf(0,	(const uint8_t *)str);
 SEGGER_RTT_printf(0,	"\r\n");
...
//    nrf_gpio_pin_clear(ADV_INTERVAL_SLOW_LED_PIN_NO);
    ...
    if (m_advertising_mode == BLE_DIRECTED_ADV)
    {
        ...
        //-----------------------
        SEGGER_RTT_printf(0, "advertising_start#BLE_DIRECT_ADV  ");
        SEGGER_RTT_printf(0, " peer_addres:  %d %d %d %d %d %d",
        		peer_address.addr[0], peer_address.addr[1], peer_address.addr[2],
        		peer_address.addr[3], peer_address.addr[4], peer_address.addr[5] );
        SEGGER_RTT_printf(0, " type: %d \r\n", peer_address.addr_type);
...
//            ble_gap_addr_t       * p_whitelist_addr[1];
           ...
           log_string("creation de la whitelist ");
        ...
        case BLE_HIDS_EVT_NOTIF_ENABLED:
        {
            ...
            if (m_in_boot_mode)
            {
                ...
                )
                {
                    ...
                    uint32_t err_code;
                    log_string("sauvegarde1 dans la whitelist ...");
            ...
            else if (p_evt->params.notification.char_id.rep_type == BLE_HIDS_REP_TYPE_INPUT)
            {
                ...
                log_string("sauvegarde2 dans la whitelist ...");
                err_code = dm_service_context_set(&m_bonded_peer_handle, &service_context);
                if (err_code != NRF_ERROR_INVALID_STATE)
                {
                    APP_ERROR_CHECK(err_code);
...
//            nrf_gpio_pin_clear(ADV_INTERVAL_SLOW_LED_PIN_NO);
            ...
            log_string("BLE_GAP_EVT_CONNECTED");
            break;

        case BLE_EVT_TX_COMPLETE:
            // Send next key event
        	log_string("BLE_EVT_TX_COMPLETE");
        ...
        case BLE_GAP_EVT_DISCONNECTED:
         ...
         //   (void) buffer_dequeue(false);
           ...
           // nrf_gpio_pin_clear(CAPS_ON_LED_PIN_NO);
            ...
            log_string("BLE_GAP_EVT_DISCONNECTED");
            advertising_start();
            break;

        case BLE_GAP_EVT_TIMEOUT:
            if (p_ble_evt->evt.gap_evt.params.timeout.src == BLE_GAP_TIMEOUT_SRC_ADVERTISEMENT)
            {  log_string(" BLE_GAP_EVT_TIMEOUT");
        ...
        case BLE_GATTS_EVT_TIMEOUT:
        	log_string(" GATTC-GATTS TIMEOUT");
        ...
        case BLE_GAP_EVT_CONN_PARAM_UPDATE:
        	SEGGER_RTT_printf(0, " evt_id: %d  ", p_ble_evt->header.evt_id);
        	log_string("BLE_GAP_EVT_CONN_PARAM_UPDATE");break;
        case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
        	SEGGER_RTT_printf(0, " evt_id: %d  ", p_ble_evt->header.evt_id);
        	log_string("BLE_GAP_EVT_SEC_PARAMS_REQUEST");break;
        case BLE_GAP_EVT_AUTH_STATUS:
        	SEGGER_RTT_printf(0, " evt_id: %d  ", p_ble_evt->header.evt_id);
        	log_string("BLE_GAP_EVT_AUTH_STATUS");break;
        case  BLE_GAP_EVT_CONN_SEC_UPDATE:
        	SEGGER_RTT_printf(0, " evt_id: %d  ", p_ble_evt->header.evt_id);
        	log_string(" BLE_GAP_EVT_CONN_SEC_UPDATE");break;
        case  BLE_GAP_EVT_SEC_INFO_REQUEST:
        	SEGGER_RTT_printf(0, " evt_id: %d  ", p_ble_evt->header.evt_id);
        	log_string(" BLE_GAP_EVT_SEC_INFO_REQUEST");break;
        default:
        	SEGGER_RTT_printf(0, "autres  ---------- evt_id: %d  ", p_ble_evt->header.evt_id);
            // No implementation needed.
            break;
    }
}
...
static void ble_stack_init(void)
{
  ...
  //  SOFTDEVICE_HANDLER_INIT(NRF_CLOCK_LFCLKSRC_XTAL_20_PPM, true);  // clock setup for dev board
...
SEGGER_RTT_printf(0, "device_manager_evt_handler    device_id: %d \r\n", p_handle->device_id);
...
static void device_manager_init(void)
{
    ...
    ///-----------
    log_string("device_manager_init");
