...
#define DEVICE_NAME                          "ww_beacon_02"                               /**< Name of device. Will be included in the advertising data. */
...
//#define APP_ADV_INTERVAL_FAST            MSEC_TO_UNITS(100, UNIT_0_625_MS)               /**< Fast advertising interval (25 ms.). */
...
//#define APP_DIRECTED_ADV_TIMEOUT         180
...
{
 //LOG("[nus_data_handler]: --");
	if(length >= 2 && p_data[0] == IR_CTRL){
		switch (p_data[1]) {
			case IR_HEAD_ON:
                 RGB_8color_set(clr_Blue);
        	     nrf_gpio_pin_set(IR_head);
        	     nrf_gpio_pin_clear(IR_gun);
        	     nrf_gpio_pin_clear(IR_back);
                 break;

			case IR_GUN_ON:
                RGB_8color_set(clr_Cyan);
       	        nrf_gpio_pin_clear(IR_head);
       	        nrf_gpio_pin_set(IR_gun);
       	        nrf_gpio_pin_clear(IR_back);
                 break;

			case IR_BACK_ON:
                RGB_8color_set(clr_Magenta);
       	        nrf_gpio_pin_clear(IR_head);
       	        nrf_gpio_pin_clear(IR_gun);
       	        nrf_gpio_pin_set(IR_back);
                 break;

			case IR_ALL_ON:
                RGB_8color_set(clr_Yollow);
       	        nrf_gpio_pin_set(IR_head);
       	        nrf_gpio_pin_set(IR_gun);
       	        nrf_gpio_pin_set(IR_back);
                 break;
 ...
 */
static void services_init_old(void)
{
    uint32_t       err_code;
...
static void advertising_start(void)
{
    ...
    if (m_advertising_mode == BLE_DIRECTED_ADV)
       {
           ...
           //-----------------------
           LOG("advertising_start#BLE_DIRECT_ADV  ");
           LOG( " peer_addres:  %d %d %d %d %d %d",
  ...
  //            ble_gap_addr_t       * p_whitelist_addr[1];
             ...
             // whitelist.addr_count = BLE_GAP_WHITELIST_ADDR_MAX_COUNT;
              whitelist.addr_count = NB_ROBOTS;
             // whitelist.irk_count  = BLE_GAP_WHITELIST_IRK_MAX_COUNT;
              whitelist.irk_count  =  NB_ROBOTS;
              whitelist.pp_addrs   = p_whitelist_addr;
              whitelist.pp_irks    = p_whitelist_irk;
              err_code = dm_whitelist_create(&m_app_handle, &whitelist);
              APP_ERROR_CHECK(err_code);
               LOG( "BLE_FAST_ADV_WHITELIST   %d  %d", whitelist.addr_count, whitelist.irk_count);
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
         ...
         //   nrf_gpio_pin_set(MCLR);
            //RGB_show_color(clr_Green);
            m_conn_handle      = p_ble_evt->evt.gap_evt.conn_handle;
            m_advertising_mode = BLE_NO_ADV;

//        	 RGB_8color_set(clr_Green);
//        	 nrf_gpio_pin_set(IR_head);
//        	 nrf_gpio_pin_set(IR_gun);
//        	 nrf_gpio_pin_set(IR_back);
            nrf_gpio_pin_set(MCLR);
     ...
     //                                    NRF_GPIO_PIN_SENSE_LOW);
      ...
      //          system_off_mode_enter();
        ...
        case BLE_GAP_EVT_CONN_SEC_UPDATE:
        	LOG("BLE_GAP_EVT_CONN_SEC_UPDATE \r\n");
        	break;
        case BLE_GAP_EVT_AUTH_STATUS:
        	LOG("BLE_GAP_EVT_AUTH_STATUS \r\n");
        	break;
        default:
            // No implementation needed.
